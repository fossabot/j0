...
const srcRoot = path.join(projectRoot, 'package');
...
function buildFile(file) {
	if (targetExts.includes(path.extname(file)) && !ignorePattern.test(file)) {
		const dir = path.relative(srcRoot, file.replace(/\/(?:index|test).*$/, ''));
		const scriptPath = path.join(srcRoot, dir, 'test', 'index.es');
...
function buildDocument(file) {
	if ((/\.test\.js$/).test(file) && !ignorePattern.test(file)) {
		...
		const scriptPath = path.join(srcRoot, dir, 'index.mjs');
		const testScriptPath = path.join(srcRoot, dir, 'test', 'index.es');
...
function startServer() {
	return Promise.all([
		[srcRoot, buildFile, {ignored: ignorePattern}],
...
function build() {
	return glob(path.join(srcRoot, '**', '*.es'), {
		nodir: true,
		ignore: [
			'**/node_modules'
		]
