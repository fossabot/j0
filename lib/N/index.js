...
class N {
	...
	modifySelection(expand, direction, anchorBB, textDirection) {
		const selection = getSelection();
		const range = new J0Range(selection.getRangeAt(0), this, textDirection);
	...
	belongsTo(anotherNode) {
		let result = false;
		this
		.forEachAncestor((node) => {
	...
	contains(anotherNode) {
		return new N(anotherNode)
		.belongsTo(this);
	}
