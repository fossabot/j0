...
class N {
	...
	modifyRange(range, alter, direction) {
		...
		function selectNearRange(range, lastRange, rect, lastRect) {
			if (!lastRange) {
				return;
			}
			let side;
			...
			const d1 = Math.abs(rect[side] - anchorRect[side]);
			const d2 = Math.abs(lastRect[side] - anchorRect[side]);
			if (d2 < d1) {
		...
		if (forward) {
			let done;
			if (character) {
				done = this.forwardRange(clonedRange);
			} else {
				let lastRange;
				let lastRect;
				while (this.forwardRange(clonedRange)) {
					const rect = clonedRange.getBoundingClientRect();
					if (
						compareRect(anchorRect, rect, crossDirection) &&
						compareRectEq(anchorRect, rect, mainDirection)
					) {
						selectNearRange(clonedRange, lastRange, rect, lastRect);
						done = true;
						break;
					}
					lastRange = clonedRange.cloneRange();
					lastRect = rect;
				}
			}
			if (!done) {
				console.error('at end', anchorRect);
			}
			range.setEnd(clonedRange.endContainer, clonedRange.endOffset);
			if (alter !== 'expand') {
				range.setStart(clonedRange.endContainer, clonedRange.endOffset);
			}
		} else {
			let done;
			if (character) {
				done = this.backwardRange(clonedRange);
			} else {
				let lastRange;
				let lastRect;
				while (this.backwardRange(clonedRange)) {
					const rect = clonedRange.getBoundingClientRect();
					if (
						compareRect(rect, anchorRect, crossDirection) &&
						compareRectEq(rect, anchorRect, mainDirection)
					) {
						selectNearRange(clonedRange, lastRange, rect, lastRect);
						done = true;
						break;
			...
			range.setStart(clonedRange.startContainer, clonedRange.startOffset);
			if (alter !== 'expand') {
				range.setEnd(clonedRange.startContainer, clonedRange.startOffset);
			}
		}
	}
