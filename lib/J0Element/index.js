...
class J0Element {
	...
	setPrevious(...elements) {
		while (0 < elements.length) {
			const element = dom(elements.shift());
	...
	setNext(...elements) {
		...
		while (0 < elements.length) {
			const element = dom(elements.pop());
	...
	replaceWith(...newChilds) {
		const lastNewChild = dom(newChilds.pop());
	...
	once(eventName, fn = this.getMethodCaller(eventName)) {
	...
	on(eventName, fn = this.getMethodCaller(eventName)) {
	...
	get focused() {
		const focusedElement = dom.find(':focus');
