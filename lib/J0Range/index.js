...
class J0Range {
	...
	getChecker(direction, anchorBB = this.bb) {
		...
		const compare = (x, y, useStart) => {
			...
			if (d < threshold) {
				candidates.push(
					useStart
					? [d, this.startContainer, this.startOffset]
					: [d, this.endContainer, this.endOffset]
				);
			}
		};
		let checkEnds;
			...
			() => {
				...
				if (0 < bb.width && 0 < bb.height) {
					...
					if (0 < canditateCount && canditateCount === candidates.length) {
						// candidates
						// .forEach(([d, container, offset]) => {
						// 	console.log(`(${targetX.toFixed(0)}, ${targetY.toFixed(0)}) ${d.toFixed(1)} ${JSON.stringify(container.textContent)} ${offset}`);
						// });
						return candidates
						.sort(([a], [b]) => {
							return a < b ? -1 : 1;
	...
	forwardLine(direction) {
		const cloned = this.clone();
		const check = cloned.getChecker(direction);
		while (1) {
			cloned.setStart(cloned.endContainer, cloned.endOffset);
			if (cloned.forwardEnd()) {
				if (0 < cloned.endOffset && !cloned.isLineBreak) {
					const result = check();
					if (result) {
