...
class J0Range {
	...
	constructor(range, container, textDirection) {
	...
	getCollapsedBB(toStart) {
		const [mainDirection] = this.textDirection || this.container.textDirection;
		const bb = this._bb;
	...
	product(direction, crossDirection) {
		const textDirection = (this.textDirection || this.container.textDirection)[crossDirection ? 1 : 0];
	...
	modify(alter, direction, anchorBB = this.bb) {
