		...
		.forEach(([visualDirection, textDirection]) => {
			...
			case 'mainF':
				it(`[${textDirectionType}] should move forward along to main axis and hit the end`, function () {
					...
					range.modify('move', visualDirection);
					range.modify('move', visualDirection);
					assert.deepEqual(this.events, ['range:last', 'range:last']);
				});
			...
			case 'mainB':
				it(`[${textDirectionType}] should move backward along to main axis`, function () {
					...
					range.modify('move', visualDirection);
					range.modify('move', visualDirection);
					assert.deepEqual(this.events, ['range:first', 'range:first']);
				});
				...
				break;
			case 'crossF':
				// it(`[${textDirectionType}] should move forward along to cross axis [start of lines]`, function () {
				// 	const range = new J0Range(null, this.element);
				// 	range.set(this.firstTextNode, 0);
				// 	this.textNodes
				// 	.slice(1)
				...
				// });
				// it(`[${textDirectionType}] should move forward along to cross axis [middle of lines]`, function () {
				// 	const range = new J0Range(null, this.element);
				...
				// 	const range = new J0Range(null, this.element);
				// 	range.set(this.firstTextNode, this.firstTextNode.textContent.trim().length);
				// 	this.textNodes
				// 	.slice(1)
				// 	.forEach((textNode, index) => {
				// 		checkRange.prefix = `[${index}]`;
				// 		range.modify('move', visualDirection);
				// 		checkRange(range, textNode, textNode.textContent.trim().length);
				// 	});
				// 	assert.deepEqual(this.events, []);
				// 	range.modify('move', visualDirection);
				// 	range.modify('move', visualDirection);
